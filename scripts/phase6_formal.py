...
def lock_hashes(value: dict, corpus: dict) -> dict[str, str]:
    ...
    locked_protocol["rq2_locked_corpus"] = corpus
...
def load(path: Path) -> tuple[dict, dict]:
    ...
    corpus = yaml.safe_load(Path(value["rq2"]["corpus_source"]).read_text(encoding="utf-8"))
    return value, corpus


def protocol(path: Path) -> dict:
    value, corpus = load(path)
    ...
    if corpus.get("phase6_formal") is not True or corpus.get("item_count") != value["rq2"]["corpus_items"] or len(corpus.get("target_rows", [])) * len(corpus.get("forms", [])) != corpus["item_count"]:
    ...
    hashes = lock_hashes(value, corpus)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "lock-hashes":
        value, corpus = load(args.config)
        print(json.dumps(lock_hashes(value, corpus), sort_keys=True))
        return 0
