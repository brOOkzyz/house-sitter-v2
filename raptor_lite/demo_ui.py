...
class DemoController:
    ...
    def plan(self, text: Any) -> dict[str, Any]:
        with self._lock:
            ...
            try:
                ...
                self.index = 0; self.paused = True; self.phase = "planned"
                return self.state()
            finally:
                self._end()
    ...
    def _run(self, seed: int, scenario: str) -> dict[str, Any]:
        ...
        self.index = 0; self.paused = True; self.phase = "playback" if self.result.success else "failed"
        return self.state()
    ...
    def playback(self, action: str) -> dict[str, Any]:
        with self._lock:
            ...
            elif action == "step": self.paused = True; self.index = min(len(self.trace), self.index + 1)
            elif action == "restart": self.paused = True; self.index = 0
