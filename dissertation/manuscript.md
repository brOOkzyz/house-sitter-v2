...
Natural-language robot programming; capability grounding; runtime verification; domestic service robots; Digital Twin; ROS 2; reproducible robotics.
...
Automated planning represents goals, state and admissible actions explicitly [cite:Ghallab2004]. Behaviour Trees offer another modular representation in which control flow and recovery remain inspectable [cite:Colledanchise2018]. RaPToR-Lite does not implement a general PDDL planner or full Behaviour Tree runtime. It adopts the narrower lesson: language output should become typed data with explicit steps, parameters, timeout and failure semantics before it is executable.
...
Large language models have widened the set of possible robot interfaces. Zero-shot planning extracts action sequences from model knowledge [cite:Huang2022]; Code as Policies produces programs that call robot APIs [cite:Liang2023]; Toolformer studies learned use of external tools [cite:Schick2023]. Multimodal systems such as PaLM-E and RT-2 connect language, images and actions [cite:Driess2023,Zitkovich2023], while RT-1 demonstrates large-scale real-robot policy learning [cite:Brohan2023RT1]. These systems provide important evidence that language can support compositional control.
...
Selected errors can receive a safe-repair proposal, such as adding an explicit return/stop sequence or replacing an unsupported default with a supported alternative. The proposal is a new TaskSpec and must be verified again; it is not silently substituted. The implementation and tests demonstrate this mechanism, but the Phase 6 raw records do not provide an auditable denominator for a formal repair-success percentage. The dissertation therefore describes repair as a feature and reports its formal empirical rate as not available.
...
House2D version 1.1 is frozen as the experimental backend. Adding Create3ROS2Backend did not change its default profile, planner behaviour, seed mapping or formal results. It remains the only backend used for Phase 6 outcome claims. Calling it a research backend rather than a simulator of Create 3 avoids attributing physical fidelity it does not possess.
...
Observation dropout is represented as an invalid or missing observation, whereas the locked event ontology expected a `missing_observation` anomaly. The primary scoring preserves that distinction and consequently counts mismatches. Phase 7 additionally evaluates a post-hoc semantic remapping, but it is never substituted for the primary metric.
...
Threshold detection is deliberately simple. A single observation can cross a temperature or humidity limit because of event magnitude or bounded noise; there is no temporal filter or learned uncertainty model in the Phase 6 core. Dropout can suppress evidence that would otherwise support another anomaly. These simplifications are central to interpreting the primary F1 of 0.403: the experiment evaluates the implemented onboard-equivalent pipeline, not an ideal event decoder.
...
The interface is a research demonstration, not a remote robot controller. During automated validation it runs locally with simulated data. It does not bypass verification, and no physical motion command is published. Archived Gazebo and earlier three-dimensional demo material are not evidence for the Phase 6 claims and are not used in the results figures.
...
## 4.2 Experimental Integrity and Freeze

The research core, House2D configuration, schemas and backend identities were frozen before formal execution. Pilot outputs were stored under separate `pilot` directories, marked `pilot_only=true`, and excluded from Phase 6 statistics. Formal products are marked `phase6_formal=true` and contain the protocol hash, analysis-plan hash, Git revision, condition, seed or paired case identifier and raw outcome. Raw records were not altered during analysis; derived tables and plots are separate artefacts.
...
The locked protocol hash is `25a16e15fc07c2c9d3c76e52de067ca47f09950ac532bbbf1f14e611753c2847`; the analysis-plan hash is `fc1e1e1e20817435a80c0886715dcb25ce4ee3844e0ecf64f15c7189f34f9594`. The RQ3 implementation-correction hash is `cae4f103e4777485397ec3b239d108c428fc20c2ebea994d1aa0c0afc9f550f9`, and the replay-normalisation addendum hash is `68dd987fe6d8a4f9fd1e06b281f119e75d2640a481bc977f06a9dcae0767d57a`. These identifiers allow the result records to be matched to the exact pre-analysis materials.
...
Micro metrics use all 320 utterances. Because the eight forms of a target are not independent semantic samples, macro means and bootstrap confidence intervals resample the 40 target groups. Decision correctness measures accept/reject/clarify agreement; intent correctness measures the normalised activity; TaskSpec exact match compares the complete normalised structure; end-to-end correctness requires the locked combination of correct decision, intent, TaskSpec and verifier outcome. Language-form slices are secondary descriptive results.

## 4.6 RQ3 Stochastic Protocol

RQ3 uses 400 fixed seeds under three paired conditions: full system, route optimisation disabled, and resource-policy counterfactual. The 100 development seeds generate 300 logical development records used only for implementation checks. The 300 held-out seeds generate 900 confirmatory logical records. Forty held-out full-system cases are selected by the locked replay rule, giving 1,240 logical records/checks overall. Development outcomes never enter the reported success rate, F1, confidence intervals, effect sizes or p-values.
...
The results answer RQ1 within the benchmark: capability grounding prevents unsupported task admission, while explicit verification prevents a larger set of structural and safety defects. The categories are complementary rather than interchangeable. A representative success is `rq1-unsupported-001`, where “Inspect the garage” is rejected because the room is absent from the profile. A representative verifier-only catch is `rq1-unsafe-001`, whose movement and inspection steps omit safe return. The full system rejects it before execution and can propose a separately verified repair, but Phase 6 does not provide a formal repair-success rate.
...
Decision correctness was 315/320 (98.4%, micro 95% CI 96.4--99.3%). Intent correctness was 286/320 (89.4%, 85.5--92.3%), TaskSpec exact match was 272/320 (85.0%, 80.7--88.5%), and end-to-end correctness was 253/320 (79.1%, 74.3--83.2%). Target-grouped macro means are identical because every target contributes eight forms, but the locked bootstrap intervals are wider: 96.9--99.7% for decision, 83.1--94.1% for intent, 73.8--94.4% for exact match and 67.8--88.8% for end-to-end correctness.
...
The author thanks Jagmohan Chauhan for supervision and guidance on the SEIoT MSc Final Project. The project brief and early RaPToR materials provided the initial motivation; all implementation, experimental claims and limitations in this report are based on the audited repository and frozen formal evidence described above.

# REFERENCES

# APPENDIX A. REPRODUCIBILITY AND ARTEFACT INDEX

The dissertation evidence pack records the frozen Git baseline, protocol and analysis hashes, RQ raw logical hashes, Phase 7 results-manifest hash, source-file hashes and every table/figure dependency. Formal RQ1, RQ2 and RQ3 records remain outside the dissertation commit and are checked byte-for-byte before and after document generation. Pilot directories are explicitly excluded.
...
[[FIGURE:figures/supp_route_effect.png|Figure A1. Supplementary paired route-cost comparison for the 300 held-out seeds. The locked estimate is full minus route-disabled = -4.737 units (95% bootstrap CI -5.186 to -4.268); completion did not change.]]

[[FIGURE:figures/supp_detection_sensitivity.png|Figure A2. Supplementary comparison of locked primary anomaly-detection F1 and the post-hoc dropout/missing-observation semantic sensitivity. The exploratory value does not replace the primary result.]]
