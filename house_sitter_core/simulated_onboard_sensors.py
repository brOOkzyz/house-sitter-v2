...
def observe_room(
    room_id: str,
    step: int,
    baseline_room: dict[str, Any],
    *,
    unexpected_obstacle: bool = False,
    ...
    values = injected_values or {}
    obstacle_count = int(values.get("obstacle_count", int(baseline_room["obstacle_count"]) + (1 if unexpected_obstacle else 0)))
    nearest_obstacle_m = float(values.get("nearest_obstacle_m", 0.28 if unexpected_obstacle else 0.80))
    return {
        ...
        "distance_observation": {"nearest_obstacle_m": nearest_obstacle_m, "obstacle_count": obstacle_count},
        "temperature_c": values.get("temperature_c", baseline_room["temperature_c"]),
        "humidity_percent": values.get("humidity_percent", baseline_room["humidity_percent"]),
        "obstacle_count": obstacle_count, "unexpected_obstacle": unexpected_obstacle,
        "layout_signature": values.get("layout_signature", baseline_room["layout_signature"]), "confidence": 0.96,
