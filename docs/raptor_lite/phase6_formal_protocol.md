...
This is Revision 3 of the pre-registered execution plan, not a result. It is pinned to frozen
...
Revision reason: pre-execution replacement of system-derived labels with
independent explicit oracle ground truth and independence guards; no formal
results existed before revision. The locked inputs are
`phase6_rq1_cases.json`, `phase6_rq2_corpus.yaml` and
`phase6_rq3_seed_manifest.json`; the independent oracle and AST-based validity
audit are `scripts/phase6_independent_oracle.py` and
`scripts/phase6_validity_audit.py`. Neither may import or call the planner,
verifier, detector, executor or any evaluated `raptor_lite` module.

RQ3 ground truth is complete direct House2D scenario-event data generated by
the independent oracle. Event time and duration were removed before execution
because frozen House2D has no input that represents them; they must not be
reported as randomized experimental factors.
