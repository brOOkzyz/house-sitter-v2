...
The CLI set is `list_simulation_skills.py`, `preview_simulation_skill.py`, and `run_simulation_skill.py`. A run publishes only `skill_request.json`, `skill_plan.json`, `skill_result.json`, `skill_events.jsonl`, and `skill_report.md` through one sibling temporary directory and rename. Input checks establish structural and internal consistency only, not cryptographic provenance or an unforgeable source. Complete internally consistent imitations remain outside the local review threat model. The base runtime and default/dry-run modes require no ROS or Nav2 and send zero action goals; real robots, hardware drivers, and physical-device commands remain unsupported. See `docs/simulation_skills.md` for the catalog and examples.

The optional `run_skill_in_gazebo.py` bridge is limited to an already-running Gazebo Sim + Nav2 environment and is disabled unless `--execute-simulation` is explicit. It sends only Nav2 `NavigateToPose` requests in the `map` frame with `use_sim_time`, using planner-retained accepted safe-goal references; it never publishes `cmd_vel` or accesses hardware, real sensors, IoT, docking, or device controls. Default and `--dry-run` execution require no ROS and send zero action goals. **This project is simulation-only and does not support real-robot deployment.**

### Offline natural-language adapter

`parse_skill_request.py` is a deterministic, offline Chinese/English front end for eight reviewed representative capabilities: patrol, all-room checking, single-area inspection, safe waiting, return-to-charger, pause, resume, and cancellation. It maps only a fixed alias vocabulary to canonical skill parameters and produces an existing `SkillRequest`; optional planner validation remains read-only. Ambiguous, conflicting, incomplete, unsupported, physical-device, and real-robot requests fail closed. No coordinate, safe-goal, ROS, Nav2, Gazebo, RViz, network, LLM, or command path is introduced. A future LLM provider may select only the same reviewed skills and parameters; it must not generate coordinates or control a robot.

`run_natural_language_skill.py` provides the corresponding unified pipeline: adapter -> existing `SkillRequest` -> existing planner -> existing execution bridge. It atomically records the original text, parse result, planner plan/status, execution mode, action-goal count, and final status. Default and `--dry-run` operation is ROS-free with zero action goals. Only explicit `--execute-simulation` may connect to a pre-existing Gazebo/Nav2 environment, and it performs parse/planner preflight before that connection. Natural language never supplies coordinates; every navigation target remains an accepted safe-goal reference selected by the planner.

### Offline execution evaluation

`evaluate_skill_execution.py` is a separate, offline-only quantitative evaluator for completed Gazebo/Nav2 execution artifacts. It accepts one or more execution directories only when their request, plan, events, and result records are structurally complete, internally consistent, and explicitly simulation-only/review-only with `real_robot_supported: false`. It deterministically aggregates per-trial skill status, sent-goal count, normalized feedback, recovery count, known navigation-time duration, timeout policy/effective timeout/basis, and terminal failure reason into CSV, JSON, and Markdown reports through one atomic directory publication. Missing, malformed, or out-of-boundary inputs fail closed and leave no partial report. The tool does not start ROS, Gazebo, Nav2, RViz, or any command path; it is intended for reproducible project and paper simulation evaluation only.
